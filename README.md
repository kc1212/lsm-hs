...
There are two levels C0 and C1 and one temporary level - I0 (Immutable C0). C0, resides only on memory, is implemented using Data.Map from the containers package (internally it is a balanced binary tree).
...
Finally, IC0 is the same as C0 except it is immutable.
...
One copy of the old C1 table will be kept on disk. There should be a file, call it CURRENT, that keeps track of the latest C1.

### Read
When the user wishes to read a value, the program looks into C0, I0 and C1 in that order and return the first result it finds, otherwise returns nothing.
